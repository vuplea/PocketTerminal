...
describe('resolveHubPasswords', () => {
  const saved = {
    webaccess: process.env.POCKETTERM_WEBACCESS_PASSWORD,
    workstation: process.env.POCKETTERM_WORKSTATION_PASSWORD,
  };
  afterEach(() => {
    if (saved.webaccess === undefined) delete process.env.POCKETTERM_WEBACCESS_PASSWORD;
    else process.env.POCKETTERM_WEBACCESS_PASSWORD = saved.webaccess;
    if (saved.workstation === undefined) delete process.env.POCKETTERM_WORKSTATION_PASSWORD;
    else process.env.POCKETTERM_WORKSTATION_PASSWORD = saved.workstation;
  });

  test('takes both passwords from the environment', () => {
    process.env.POCKETTERM_WEBACCESS_PASSWORD = 'web-password-long-enough';
    process.env.POCKETTERM_WORKSTATION_PASSWORD = 'node-password-long-enough';
  ...
  test('reports every problem at once, naming the variable', () => {
    process.env.POCKETTERM_WEBACCESS_PASSWORD = 'change-me-long-random';
    process.env.POCKETTERM_WORKSTATION_PASSWORD = 'short';
    const { problems } = resolveHubPasswords();
    expect(problems).toHaveLength(2);
    expect(problems[0]).toContain('POCKETTERM_WEBACCESS_PASSWORD');
    expect(problems[1]).toContain('POCKETTERM_WORKSTATION_PASSWORD');
  });
  ...
  test.skipIf(isWindows)('a missing variable is a problem, not a throw', () => {
    delete process.env.POCKETTERM_WEBACCESS_PASSWORD;
    delete process.env.POCKETTERM_WORKSTATION_PASSWORD;
    const { problems } = resolveHubPasswords();
    expect(problems).toHaveLength(2);
    expect(problems[0]).toContain('Missing POCKETTERM_WEBACCESS_PASSWORD');
  });
});
